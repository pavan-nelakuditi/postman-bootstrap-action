...
describe('PostmanAssetsClient', () => {
  ...
  it('preserves non-autogenerated test events when injecting smoke tests', async () => {
    const fetchImpl = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(
        jsonResponse({
            ...
            item: [
              {
                ...
                request: {
                  method: 'GET',
                  url: { raw: '{{baseUrl}}/products/:id' }
                },
                event: [
                  {
                    listen: 'test',
                    script: {
                      exec: ['// [FlowExtract] Auto-generated response extractors', 'pm.collectionVariables.set("productId", "x");']
                    }
                  }
                ]
              }
    ...
    await client.injectTests('col-smoke', 'smoke');

    const updateCall = fetchImpl.mock.calls[1];
    const payload = JSON.parse(String(updateCall?.[1]?.body || '{}'));
    const requestEvents = payload.collection.item[1].event;
    expect(requestEvents).toHaveLength(2);
    expect(requestEvents[0].script.exec[0]).toContain('[FlowExtract]');
    expect(requestEvents[1].script.exec[0]).toContain('[Smoke]');
  });
